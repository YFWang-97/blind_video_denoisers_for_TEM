...
class denoiser():
    def load_model(self, lr = 1e-4):
        args = argparse.Namespace(model='blind-spot-net', 
                                in_channels=1,
                                out_channels=1,
                                bias=False, 
                                normal=False, 
        ...
        for epoch in range(num_epochs):
            ...
            for sample in valid_bar:
                with torch.no_grad():
                    ...
                    valid_psnr = utils.psnr(sample, outputs, False)
                    valid_ssim = utils.ssim(sample, outputs, False)
                    valid_meters["valid_psnr"].update(valid_psnr.item())
                    valid_meters["valid_ssim"].update(valid_ssim.item())
                    
                    loss_avg += F.mse_loss(outputs, sample) / batch_size
