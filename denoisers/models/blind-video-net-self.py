...
from denoisers.models import register_model
...
@register_model("blind-video-net-self")
class BlindVideoNet(nn.Module):
    def __init__(self, channels_per_frame=3, out_channels=9, bias=False, blind=True, sigma_known=True):
        ...
        self.rotate = rotate()
        self.denoiser_1 = Blind_UNet(n_channels=channels_per_frame, n_output=32, bias=bias, blind=blind)
        self.denoiser_2 = Blind_UNet(n_channels=32, n_output=32, bias=bias, blind=blind)
        if not sigma_known:
            self.sigma_net = Blind_UNet(n_channels=5*channels_per_frame, n_output=1, bias=False, blind=False)
        if blind:
            self.shift = shift()
        self.unrotate = unrotate()
        self.nin_A = nn.Conv2d(128, 384, 1, bias=bias)
