...
def save_checkpoint(args, step, model, optimizer=None, scheduler=None, score=None, mode="min"):
    ...
    if not args.no_save and step % args.save_interval == 0:
        ...
        if (score < best_score and mode == "min") or (score > best_score and mode == "max"):
            torch.save(state_dict, os.path.join(args.checkpoint_dir, ''.join(args.data_path.split('/')[-1].split('.')[-2]) + "_best.pt"))
        if step > last_step:
            torch.save(state_dict, os.path.join(args.checkpoint_dir, ''.join(args.data_path.split('/')[-1].split('.')[-2]) + "_last.pt"))
